...
### コンテンツ
- [ ] `_pages/terms.md` を更新 — 2016年テンプレのまま。Disqus・Google Analytics への言及があるが両方オフ。現状に合わせるか削除する
...
### 設定
- [ ] `_config.yml` の `teaching`・`portfolio` コレクション定義を削除 — 中身が空なので定義ごと消してもよい
...
- [ ] `_config.yml` に ORCID を追加 — 取得したら `orcid:` に設定
- [ ] `_config.yml` に ResearchGate・LinkedIn を追加 — 使っていれば
- [ ] `_config.yml` の `bio` を更新 — 現在「Master student @ The University of Tokyo, Pharmaceutical Sciences」のみ。研究内容（NLP・毒性予測など）を一言加えると良い
...
### 見た目・細かい修正
- [ ] `_pages/about.md` の「Selected Activities」見出しを修正 — `------` アンダーラインがなく他の見出しと形式が揃っていない
- [ ] ファビコン・OGP 画像を個人用に差し替え — 現在はテンプレのデフォルト画像
- [ ] `_data/authors.yml` の bio を `_config.yml` と統一 — 現在「機械学習・バイオインフォマティクス・データ可視化」で研究内容の表現が微妙に異なる

---

## 📄 論文・発表が出たら

- [ ] `_publications/` に論文・プレプリントを追加 — `HOW_TO_EDIT.md` の命名規則を参照
- [ ] `_pages/about.md` の「現在準備中」を実際の citation に差し替え
- [ ] `_pages/cv.md` の「特になし」を更新
